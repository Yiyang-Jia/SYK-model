#studies the overshoot Abi mentioned, using 4 as an example
...
unfoldEnergies = readdlm("n=24q=4unfoldedLevels(trapezoidMethod)")
...
open("n=24q=4SFFconnectedDataHalfEigenVals","w") do  sffconfileTrunc1
    writedlm(sffconfileTrunc1, sffConUnfTrunc1)
end
sffFullSpec = readdlm("/home/yiyang/Desktop/Codes/julia/n=24q=4SFFconnectedUnfoldedData")
plot( sffConUnfTrunc1[: , 1], [sffFullSpec[:,2] sffConUnfTrunc1[: , 2]] , label = nothing)
### conlusion : no overshoot
...
open("n=24q=4SFFconnectedDataOneThirdEigenVals","w") do  sffconfileTrunc2
...
open("n=24q=4SFFconnectedDataOneQuarterEigenVals","w") do  sffconfileTrunc3
...
plot(decay1[:,1] , [decay1[:, 2]  decay3[:, 2]] , label = ["half of unfolded spectrum" "a quarter of unfolded spectrum"], title = "SYK_4 (N=24)  SFF overshoot region", xlabel = "t")
#savefig("SYK2overshoot.pdf")

plot(log.(decay1[:,1]) , log.([decay1[:, 2]  decay3[:, 2]]) , label = ["half of unfolded spectrum" "a quarter of unfolded spectrum"], title = "SYK_4 (N=24)  SFF overshoot region loglog plot", xlabel = "t")
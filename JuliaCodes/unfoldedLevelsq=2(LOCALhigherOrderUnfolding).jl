...
begin
    ...
    for k = 1 : nens
        specDenFit = sixthOrderFit(n, evsEns8000realizations[:,k], multiPstadev, eta, emax)
        unfoldedEnergyLevels[:,k] =unfoldTrapz(specDenFit,evsEns8000realizations[:,k])
        if mod(k, 200) == 0
            println(k)
        end
    end 
    open("n=24q=2unfoldedLevels(6thOrderLocalUnfoldingWithOddTerms)0p9Bulk","w") do  ufhquadfile
